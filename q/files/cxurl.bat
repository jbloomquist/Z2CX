...
POWERSHELL -NoLogo -NoProfile -Command "$query='%~1';$max=(Get-ChildItem '%PAGEPREFIX%-*.html').Count;$page=1;$i=0;[System.Console]::TreatControlCAsInput=$true;$esc=[char]27;$mint=$esc+'[38;5;121m';$reset=$esc+'[0m';while($true){$items=Get-Content ('%PAGEPREFIX%-'+$page+'.html') | ConvertFrom-Json;Clear-Host;Write-Host $query;Write-Host ('-'*($query.Length));for($j=0;$j -lt $items.Count;$j++){if($j -eq $i){Write-Host ($mint+'> '+$items[$j].Text+$reset)}else{Write-Host ('  '+$items[$j].Text)}};Write-Host '';Write-Host ('-- Page '+$page+' of '+$max+' --');Write-Host '';Write-Host '[Up/Down = move]  [Enter = open/download]  [Right = next page]  [Left = prev page]  [Esc = quit]';$key=$Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown');if($key.VirtualKeyCode -eq 38 -and $i -gt 0){$i--};if($key.VirtualKeyCode -eq 40 -and $i -lt $items.Count-1){$i++};if($key.VirtualKeyCode -eq 39){$page++;if($page -gt $max){$page=1};$i=0};if($key.VirtualKeyCode -eq 37){$page-- ;if($page -lt 1){$page=$max};$i=0};if($key.VirtualKeyCode -eq 27){exit};if($key.VirtualKeyCode -eq 13){$u=$items[$i].Url;if($u -match '\.(png|jpe?g|gif|bmp|webp|svg|mp3|wav|ogg|flac|mp4|mkv|webm|avi|mov|txt|log|rtf|bat|cmd|sh|bash|ps1|zip|7z|rar|tar|gz|bz2|tgz|xz)$'){$fn=[System.IO.Path]::GetFileName($u);$dl=Join-Path $env:USERPROFILE 'Downloads';if(-not(Test-Path $dl)){New-Item -ItemType Directory -Path $dl|Out-Null};& curl.exe -s $u -o (Join-Path $dl $fn);Write-Host ('Downloaded: '+(Join-Path $dl $fn));exit}else{$auth='%~2';if([string]::IsNullOrWhiteSpace($auth)){& '%~f0' $u}else{& '%~f0' $u $auth};exit}}}"
